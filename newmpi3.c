...
//master
	...
	newOffset=source->altura/nproc;
	
	imagesize=source->altura*source->ancho;
	chunksize1=(newOffset*source->ancho)/partitions;
...
//	freeImagestructure(&source);
  //      freeImagestructure(&output);
	...
	convolve2D(output1->R, output2->R, source->ancho, (newOffset/partitions)+halosize, kern->vkern, kern->kernelX, kern->kernelY);
        convolve2D(output1->G, output2->G, source->ancho, (newOffset/partitions)+halosize, kern->vkern, kern->kernelX, kern->kernelY);
        convolve2D(output1->B, output2->B, source->ancho, (newOffset/partitions)+halosize, kern->vkern, kern->kernelX, kern->kernelY);
